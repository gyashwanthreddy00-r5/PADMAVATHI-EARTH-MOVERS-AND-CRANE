...
<style>
  * { box-sizing: border-box; }
  ...
  const sendBalanceStatement = async () => {
    ...
    try {
      // Rendered server-side via Browserless (real Chromium), the same path the
      // single-invoice Email action uses, so each PDF is pixel-identical to its Print view.
      ...
      const { data, error } = await supabase.functions.invoke('send-balance-statement', {
        body: { customerId: selectedCustomer.id, invoiceIds: balanceStatementRows.map(r => r.id), pdfHtmls, statementLabel: 'Balance Statement' },
      });
      if (error) {
