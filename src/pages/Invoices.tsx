...
<style>
  * { box-sizing: border-box; }
  ...
  const sendBalanceStatement = async () => {
    ...
    try {
      // Attachments reuse existing PDF generation exactly — buildStatementHtml() (via
      // htmlToPdfBase64) and the same generateInvoicePdfBase64 already used by the
      // per-invoice Email action — nothing new is generated here.
      ...
      const { data, error } = await supabase.functions.invoke('send-balance-statement', {
        body: { customerId: selectedStatementCustomer.id, invoiceIds: balanceStatementRows.map(r => r.inv.id), attachments, statementLabel: 'Balance Statement' },
      });
      if (error) {
  ...
  const sendFullStatement = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('send-balance-statement', {
        body: { customerId: selectedStatementCustomer.id, invoiceIds: statementRows.map(r => r.inv.id), attachments, statementLabel: 'Full Statement' },
      });
      if (error) {
